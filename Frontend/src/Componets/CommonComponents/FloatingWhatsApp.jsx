...
const FloatingWhatsApp = () => {
  ...
  return (
    <aside
      ...
      className="fixed bottom-5 left-5 z-40 flex items-center sm:bottom-6 sm:left-6"
    >
      <a
        href={whatsappUrl}
        target="_blank"
      ...
      {/* Tooltip banner sliding to the right */}
      <div
        className={`pointer-events-none absolute left-full ml-3 hidden items-center whitespace-nowrap rounded-xl border border-white/10 bg-[#11171c]/95 px-3.5 py-2 text-xs text-white shadow-[0_8px_25px_rgba(0,0,0,0.6)] backdrop-blur-md transition-all duration-300 sm:flex ${
          isHovered
      ...
      >
        ...
        {/* Tooltip arrow */}
        <span className="absolute -left-1.5 top-1/2 h-3 w-3 -translate-y-1/2 rotate-45 border-b border-l border-white/10 bg-[#11171c]/95" />
      </div>
    </aside>
  );
};
