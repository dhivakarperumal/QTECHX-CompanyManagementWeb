...
/* ── page title map ── */
const pageInfo = {
  "/trainee":                   { title: "Dashboard",        icon: LayoutDashboard },
  "/trainee/attendance":        { title: "My Attendance",    icon: ClipboardCheck },
  "/trainee/leaves":            { title: "My Leave",         icon: CalendarOff },
  "/trainee/projects":          { title: "My Projects",      icon: FolderKanban },
  "/trainee/tasks":             { title: "My Tasks",         icon: CheckSquare },
  "/trainee/timesheet":         { title: "Timesheet",        icon: Clock },
  "/trainee/payroll":           { title: "Salary & Payroll", icon: DollarSign },
  "/trainee/meetings":          { title: "Meetings",         icon: Video },
  "/trainee/settings":          { title: "Settings",         icon: Settings },
  "/trainee/settings/profile":  { title: "Profile",          icon: User },
};
...
/* ══════════════════ MAIN HEADER ══════════════════ */
const TraineeHeader = ({ onMenuClick }) => {
  ...
  const handleSearch = (e) => {
    ...
    navigate(`/trainee/tasks?search=${encodeURIComponent(searchQuery)}`);
    setShowSearch(false);
    setSearchQuery("");
  };
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {activeDropdown === "quick" && (
              <div className="absolute right-20 top-full mt-3 w-48 bg-[#13141a] border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden p-1.5 space-y-0.5">
                {[
                  { label: "Apply Leave",   path: "/trainee/leaves/apply",    icon: CalendarOff  },
                  { label: "Log Hours",     path: "/trainee/timesheet/log",    icon: Clock        },
                  { label: "My Tasks",      path: "/trainee/tasks",            icon: CheckSquare  },
                  { label: "Meetings",      path: "/trainee/meetings",         icon: Video        },
                  { label: "My Pay Slips",  path: "/trainee/payroll/slips",    icon: DollarSign   },
                ].map(l => {
                  const LIcon = l.icon;
                  return (
              ...
              {activeDropdown === "profile" && (
                <div className="absolute right-0 top-full mt-3 w-56 bg-[#13141a] border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden">
                  ...
                  <div className="p-1.5 space-y-0.5">
                    <Link to="/trainee/settings/profile" onClick={() => setActiveDropdown(null)}
                    ...
                    <Link to="/trainee/settings" onClick={() => setActiveDropdown(null)}
                      className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/8 text-sm text-white/80 hover:text-white transition">
                      <Settings size={15} className="text-white/40" /> Settings
                    </Link>
                    <Link to="/trainee/payroll/slips" onClick={() => setActiveDropdown(null)}
                      className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/8 text-sm text-white/80 hover:text-white transition">
          ...
          <div ref={searchRef} className="w-full max-w-2xl bg-[#13141a] border border-white/15 rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-3">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  { label: "My Tasks",     path: "/trainee/tasks" },
                  { label: "Attendance",   path: "/trainee/attendance" },
                  { label: "Apply Leave",  path: "/trainee/leaves/apply" },
                  { label: "Timesheet",    path: "/trainee/timesheet" },
                  { label: "My Projects",  path: "/trainee/projects" },
                  { label: "Pay Slips",    path: "/trainee/payroll/slips" },
                ].map(l => (
