...
const AVATAR_COLOURS = [
  '#ef4444', '#f43f5e', '#f97316', '#f59e0b', '#8b5cf6',
...
export default function CancelledTasksPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Failed to load cancelled tasks.');
  ...
  const thisMonth = useMemo(() => {
    const now = new Date();
    return tasks.filter(t => {
      // For cancelled tasks, we might just look at created_at or updated_at if completion date is null.
      // Assuming they might not have completion_date. We will use assignment_date or due_date as fallback.
      const dateToCheck = t.completion_date || t.due_date || t.assignment_date;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Cancelled Tasks</h1>
              <p className="text-xs text-white/35 mt-0.5">{tasks.length} tasks cancelled</p>
            </div>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-1 bg-[#111318] border border-white/8 rounded-xl p-1 self-start sm:self-auto">
          <button
            ...
            className={`rounded-lg p-2 transition-all ${viewMode === 'table' ? 'bg-rose-500 text-white' : 'text-white/45 hover:text-white'}`}
          ...
          <button
            ...
            className={`rounded-lg p-2 transition-all ${viewMode === 'card' ? 'bg-rose-500 text-white' : 'text-white/45 hover:text-white'}`}
      ...
      {loading ? (
        <div className="flex min-h-[45vh] items-center justify-center rounded-2xl border border-white/8 bg-[#111318]">
          <div className="flex flex-col items-center gap-3 text-white/40">
            ...
            <p className="text-sm">Loading cancelled tasks…</p>
          </div>
        </div>
      ) : error ? (
        ...
        <div className="rounded-2xl border border-dashed border-white/8 bg-[#111318] px-4 py-16 text-center">
          <div className="w-16 h-16 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center mx-auto mb-4">
            <Trash2 size={28} className="text-rose-400/50" />
          </div>
          <p className="text-white/40 text-sm">
            {search ? 'No cancelled tasks match your search.' : 'No cancelled tasks found.'}
          </p>
        </div>
      ) : viewMode === 'table' ? (
        ...
        <div className="overflow-hidden rounded-2xl border border-white/8 bg-[#111318]">
          ...
          {/* rose accent top bar */}
          <div className="h-0.5 bg-gradient-to-r from-rose-500/0 via-rose-500/60 to-rose-500/0" />
              ...
              <tbody>
                {visible.map((task, i) => (
                  <tr key={task.uuid} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-5 py-4 text-white/50 text-[13px] whitespace-nowrap">
                      {task.project_name ? (
                        <span className="flex items-center gap-1.5">
                          <FolderKanban size={12} className="text-rose-400/50 shrink-0" />
                          {task.project_name}
                        </span>
                      ) : '—'}
                    </td>
                    ...
                    <td className="px-5 py-4 text-white/50 text-[13px] whitespace-nowrap">
                      <span className="flex items-center gap-1.5">
                        <Clock size={12} className="text-rose-400/50 shrink-0" />
          ...
          <div className="px-5 py-3 border-t border-white/5 text-[11px] text-white/30">
            Showing {visible.length} of {tasks.length} cancelled tasks
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {visible.map((task, i) => (
            <div
              key={task.uuid}
              className="rounded-2xl border border-rose-500/12 bg-[#111318] p-5 flex flex-col gap-4 hover:border-rose-500/25 transition-all group"
            >
              {/* top */}
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="inline-flex items-center gap-1 text-[9px] font-bold px-2 py-1 rounded-full bg-rose-500/15 text-rose-400 border border-rose-500/25 shrink-0">
