...
const EmployeeProfile = () => {
  ...
  const employeeCode = emp.employee_code || user?.employee_code || user?.employee_id || user?.user_id || "EMP-01";
  const userFullName = emp.first_name
    ? `${emp.first_name} ${emp.last_name || ""}`.trim()
    : profileName || user?.username || "Employee Profile";
  const userRole = emp.role || role || user?.role || "Employee";
  const userDesignation = emp.designation || user?.designation || (userRole ? `${userRole}` : "Team Member");
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="rounded-2xl border border-white/10 bg-[#111318] p-5 shadow-lg space-y-3">
            ...
            <p className="text-xs text-white/50 leading-relaxed">
              Keep your employee account secure by updating your credentials periodically.
            </p>
            <button
              onClick={() => setShowPasswordModal(true)}
          ...
          <div className="rounded-2xl border border-white/10 bg-[#111318] p-5 sm:p-6 shadow-lg">
            ...
            {educationalList.length > 0 ? (
              ...
              <div className="rounded-xl border border-dashed border-white/10 p-8 text-center text-white/40">
                ...
                <p className="text-xs text-white/30 mt-0.5">Qualifications can be updated via profile administrator</p>
              </div>
            )}
          </div>
